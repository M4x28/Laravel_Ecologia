...
@section('content')
    <div class="sfondo">
        <div class="d-flex justify-content-center testoComune">
            <div class="row gx-5">
                ...
                <div class="col-lg-8">
                    <div class="row text-black h5">{{ __('COMUNE DI') }}</div>
    ...
    <div class="display-5 text-center text-primary m-5">{{ __('Benvenuto nel comune di') }} {{ $paese }}</div>
    ...
    <div class="container card d-flex justify-content-center bg-light bg-gradient shadow-custom">
        <div class="row">
            ...
            <div class="col-md-6 text-center mt-5 mb-4">
                <h2 class="text-info">{{ __('Informazioni sul Comune') }}</h2>
                <h4 class="text-black mt-4">{{ __('Regione:') }} {{ $info_comune[0]->regione }}</h4>
                <h4 class="text-black">{{ __('Provincia:') }} {{ $info_comune[0]->provincia }}</h4>
                <h4 class="text-black">{{ __('Superficie:') }} {{ $info_comune[0]->superficie }}</h4>
                <h4 class="text-black">{{ __('Residenti:') }} {{ $info_comune[0]->num_residenti }}</h4>
                <h5 class="text-black">{{ __('Indirizzo Municipio:') }} {{ $info_comune[0]->indirizzo }}</h5>
            </div>
        </div>
    </div>
    ...
    <div class="container card d-flex justify-content-center bg-light bg-gradient shadow-custom mt-5">
        <div class="row">
            ...
            <div class="col-md-6 text-center mt-5">
                <h2 class="text-warning">{{ __('Informazioni sul CCR') }}</h2>
                <h4 class="text-black mt-4">{{ __('Nome:') }} {{ $ccr[0]->nome }}</h4>
                <h4 class="text-black mt-4">Email: {{ $ccr[0]->email }}</h4>
                <h4 class="text-black mt-4">{{ __('Indirizzo:') }} {{ $ccr[0]->indirizzo }}</h4>
            </div>
        </div>
        <hr>
        <div class="row text-center mt-2 mb-4">
            <h2 class="text-info">{{ __('Orari di apertura') }}</h2>
            <h5 class="text-black m-3">{{ __('Lunedì-Venerdì:') }}
                {{ $ccr[0]->AperturaSett }}-{{ $ccr[0]->ChiusuraSett }}
            </h5>
            @if ($ccr[0]->AperturaSab != null)
                <h5 class="text-black m-3">{{ __('Sabato:') }}
                    {{ $ccr[0]->AperturaSab }}-{{ $ccr[0]->ChiusuraSab }}</h5>
            @endif
            @if ($ccr[0]->AperturaDom != null)
                <h5 class="text-black m-3">{{ __('Domenica:') }}
    ...
    <div class="container card d-flex justify-content-center bg-light bg-gradient shadow-custom mt-5">
        <h2 class="text-secondary text-center mt-4">{{ __('Informazioni sulle Zone') }}</h2>
        ...
        @foreach ($zone as $zona)
            <div class="row">
                ...
                <div class="col-md-6 text-center mt-4">
                    <h4 class="text-primary">{{ __('Zona:') }} {{ $zone[0]->nome }}</h4>
            ...
            <div class="d-flex justify-content-center m-3">
                <a class="btn btn-outline-info btn-rounded btn-lg"
                    ...
                    <strong>{{ __('Scarica Calendario') }}</strong>
                </a>
            </div>
            <hr>
        @endforeach
